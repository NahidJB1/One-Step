...
const baseDir = "C:\\Users\\User\\Desktop\\Projects\\One-Step\\Universities";
...
<head>
  ...
  <link rel="stylesheet" href="../assets/css/universities/__CSS__">
</head>
<body>
  ...
  <header class="uni-header">
    ...
    <div class="container uni-header-content">
      <div data-aos="fade-up" data-aos-duration="1000">
        <h1 class="uni-title">__TITLE__</h1>
        <div class="uni-meta">
          <span class="meta-tag"><i class="fas fa-map-marker-alt"></i> __LOCATION__</span>
          <span class="meta-tag"><i class="fas fa-star"></i> __STARS__</span>
          <span class="meta-tag"><i class="fas fa-university"></i> __EST__</span>
        </div>
      </div>
    </div>
  </header>
      ...
      <div class="sidebar-info">
        ...
        <div class="info-card glass-card" data-aos="fade-left" data-aos-delay="400">
          ...
          <div class="action-buttons">
            <a href="__FEES_LINK__" class="btn-action secondary">
...
universities.forEach(uni => {
    let fac_html = uni.faculties.map(f => `<li><i class='fas fa-check-circle'></i> ${f}</li>`).join("");
    let rnk_html = uni.rankings.map(r => `<li><i class='fas fa-award'></i> ${r}</li>`).join("");
    
    let content = template
        .replace(/__TITLE__/g, uni.title)
        .replace(/__CSS__/g, uni.css)
        .replace(/__LOCATION__/g, uni.location)
        .replace(/__STARS__/g, uni.stars)
        .replace(/__EST__/g, uni.est)
        .replace(/__OVERVIEW__/g, uni.overview)
        .replace(/__FACULTIES__/g, fac_html)
        .replace(/__LOCATION_TEXT__/g, uni.location_text)
        .replace(/__RANKINGS__/g, rnk_html)
        .replace(/__FEES_LINK__/g, uni.fees_link);
        
    fs.writeFileSync(path.join(baseDir, uni.file), content, 'utf8');
});

console.log("Generated standard university pages successfully.");
